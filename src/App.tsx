      ...
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <a href="https://calendly.com/reavan/connect-with-rathnavel" target="_blank" rel="noopener noreferrer"
            className="font-sans text-sm px-4 py-2 rounded border text-white transition-all duration-300 ml-1"
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <div className="flex-1">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              className="inline-flex items-center gap-2 mb-6 px-3 py-1 rounded-full border text-xs font-sans tracking-widest uppercase flex-wrap"
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }}
              className="font-body text-lg text-gray-700 leading-relaxed max-w-lg mb-8">
              In few words, <strong>FEW</strong> is the word.
              A <strong>Facilitator</strong> for short learning sessions, activity based learning and quiz competitions.
              An <strong>Educator</strong> formally engaged in teaching Civil Engineering.
              A <strong>Writer</strong> with adequate intelligence to compete with Artificial Intelligence.
            </motion.p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {programmes.map((p, i) => (
            <motion.div key={p.code}
              ...
              whileHover={{ y: -10, scale: 1.03, boxShadow: '0 16px 40px rgba(201,168,76,0.18)' }}
              className="rounded-xl p-6 text-center border border-white/10 cursor-default"
              ...
              <div className="font-sans text-xs text-gray-600 leading-snug">{p.sub}</div>
            </motion.div>
          ))}
        </div>
      </div>
...
const statsData = [
  { raw: 26,    suffix: '',  label: 'Years in Facilitation', emoji: '⏳' },
  { raw: 50,    suffix: '+', label: 'Organisations',         emoji: '🏢' },
  { raw: 100,   suffix: '+', label: 'Quizzes',              emoji: '🎤' },
  { raw: 200,   suffix: '+', label: 'Sessions',             emoji: '🏫' },
  { raw: 300,   suffix: '+', label: 'MoCs',                 emoji: '🎙️' },
  { raw: 10000, suffix: '+', label: 'Participants',         emoji: '👥' },
]

function StatCard({ s, active }: { s: typeof statsData[0]; active: boolean }) {
  const count = useCounter(s.raw, active)
  const display = s.raw === 10000 ? count.toLocaleString() : count.toString()
  return (
    <div className="rounded-xl p-5 text-center border border-gray-100 hover:border-yellow-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
      <div className="text-xl mb-2">{s.emoji}</div>
      <div className="font-display text-2xl font-bold mb-0.5" style={{ color: GOLD }}>{display}{s.suffix}</div>
    ...
    <section ref={ref} className="py-16 bg-white border-y border-gray-100">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {statsData.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.08, duration: 0.5 }}>
              <StatCard s={s} active={v}/>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="hidden md:block relative" style={{ minHeight: `${milestones.length * 160}px` }}>
          <svg className="absolute inset-0 w-full h-full" style={{ zIndex:0 }} preserveAspectRatio="none"
            ...
            <path d="M450,40 C650,40 750,200 450,240 C150,280 50,440 450,480 C750,520 650,680 450,720 C150,760 50,920 450,960 C750,1000 650,1160 450,1200 C150,1240 50,1400 450,1440 C750,1480 650,1640 450,1680 C150,1720 250,1840 450,1880"
              fill="none" stroke="#CBD5E0" strokeWidth="3" strokeDasharray="10 6" filter="url(#sketch)" strokeLinecap="round"/>
          </svg>
          {milestones.map((m, i) => {
            ...
            return (
              <motion.div key={i}
                ...
                style={{ position:'absolute', top:`${(i / milestones.length) * 100 + 1}%`, left: isLeft ? '2%' : '52%', width:'42%', zIndex:2 }}>
                <div style={{ position:'relative', background:m.bg, border:`1.5px solid ${m.color}33`, borderRadius:'12px', padding:'20px 20px 16px 22px', boxShadow:`3px 4px 0px ${m.color}22, 0 2px 12px rgba(0,0,0,0.06)` }}>
                  <div style={{ position:'absolute', top:6, right:8, width:28, height:28, borderTop:`2px solid ${m.color}40`, borderRight:`2px solid ${m.color}40`, borderRadius:'0 6px 0 0' }}/>
                  <Pushpin color={m.color}/>
                  <div style={{ color:m.color, fontFamily:'"Playfair Display",Georgia,serif', fontSize:'1.4rem', fontWeight:700, marginBottom:2 }}>{m.year}</div>
                  <div style={{ color:'#111827', fontFamily:'"Playfair Display",Georgia,serif', fontSize:'1.05rem', fontWeight:700, marginBottom:3, lineHeight:1.3 }}>{m.role}</div>
                  <div style={{ color:'#6B7280', fontFamily:'"Manrope",sans-serif', fontSize:'0.78rem', lineHeight:1.4 }}>{m.org}</div>
                </div>
                <div style={{ position:'absolute', top:'50%', [isLeft ? 'right' : 'left']:'-36px', transform:'translateY(-50%)', display:'flex', alignItems:'center' }}>
        ...
        <div className="md:hidden space-y-5">
          {milestones.map((m, i) => (
            <motion.div key={i} initial={{ opacity:0, x:-20 }} animate={v ? { opacity:1, x:0 } : {}} transition={{ delay: i * 0.06, duration: 0.5 }}
              className="flex gap-3 items-start">
              <div className="flex-shrink-0 mt-1"><RoadPin color={m.color}/></div>
              <div style={{ flex:1, background:m.bg, border:`1.5px solid ${m.color}30`, borderRadius:'10px', padding:'14px 16px', boxShadow:`2px 3px 0px ${m.color}20` }}>
...
const advisoryClients = [
  ...
  { name: 'IRAAH Photography',               url: 'https://www.instagram.com/iraahphotography/',                                      abbr: 'IR'  },
]
function Advisory() {
  const { ref, v } = useReveal()
  return (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {advisoryClients.map((c, i) => (
            <motion.a key={i} href={c.url} target="_blank" rel="noopener noreferrer"
              initial={{ opacity: 0, y: 24 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1, duration: 0.5 }}
              whileHover={{ y: -8, scale: 1.03 }}
              className="group flex flex-col items-center justify-center gap-3 p-6 rounded-xl border border-white/10"
              style={{ backgroundColor: '#ffffff' }}>
              <div className="w-14 h-14 rounded-lg border border-yellow-200 flex items-center justify-center">
                <span className="font-display text-base font-bold text-yellow-600">{c.abbr}</span>
              </div>
              <div className="text-center">
                <div className="font-display text-sm text-gray-900 font-semibold leading-tight">{c.name}</div>
                <div className="flex items-center justify-center gap-1 mt-1.5">
                  <ExternalLink size={10} className="text-gray-400 group-hover:text-yellow-500 transition-colors"/>
                  <span className="font-sans text-xs text-gray-400 group-hover:text-yellow-500 transition-colors">Visit</span>
                </div>
              </div>
            </motion.a>
          ))}
  ...
  return (
    <footer className="py-8 border-t border-white/8" style={{ backgroundColor: DARK }}>
      <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <a href="https://www.linkedin.com/in/raja-parasuraman/" target="_blank" rel="noopener noreferrer"
          className="font-sans text-white/30 text-xs hover:text-yellow-400 transition-colors text-right">
          © URGE (Untiring Raja Gopal Effort)
        </a>
      </div>
    </footer>
  )
